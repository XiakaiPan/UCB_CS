...
int bear_git_commit(char *msg)
{
    ...
    char new_file_name[FILENAME_SIZE];
    // Copy .bear_git/.index to new .bear_git/<newid>/.index
    sprintf(new_file_name, "%s/.index", commit_dir);
    fs_cp(".bear_git/.index", new_file_name);

    // Copy .bear_git/.prev to .bear_git/<newid>/.prev
    sprintf(new_file_name, "%s/.prev", commit_dir);
    fs_cp(".bear_git/.prev", new_file_name);

    /* Copy other tracked files indexed by .bear_git/.index */
    FILE *findex = fopen(".bear_git/.index", "r");
    char index_ptr[FILENAME_SIZE];
    char origin_file[FILENAME_SIZE];
    while (fgets(index_ptr, sizeof(index_ptr), findex))
    {
        /**
         *  get every indexed file's origin name from .bear_git/.index
         *  to origin_file using fgets() and index_ptr
         */
        strtok(index_ptr, "\n");
        sprintf(origin_file, "./%s", index_ptr);
        /**
         * Copy indexed file named origin_file to .bear_git/<newid>
         */
        sprintf(new_file_name, "%s/%s", commit_dir, )
    }
