...
int checkout_commit(const char *commit_id)
{
    ...
    if (strcmp(commit_id, "0000000000000000000000000000000000000000") == 0)
    {
        write_string_to_file(".bear_git/.index", "");
        return 0;
    }
    ...
    sprintf(tmp1, ".bear_git/%s/.index", commit_id);
    fs_cp(tmp1, ".bear_git/.index");

    char tmp2[FILENAME_SIZE];
    findex = fopen(".bear_git/.index", "r");
    while (fgets(line, sizeof(line), findex))
    {
        strtok(line, "\n");
        sprintf(tmp1, ".bear_git/%s/%s", commit_id, line);
        sprintf(tmp2, "./%s", line);
        fs_cp(tmp2, tmp2);
    }
...
int bear_git_checkout(const char *arg, int new_branch)
{
    ...
    char current_branch[BRANCHNAME_SIZE];
    read_string_from_file(".bear_git/.current_branch", current_branch, BRANCHNAME_SIZE); //1
    ...
    if (!strlen(current_branch))
    {
        ...
        sprintf(current_branch_file, ".bear_git/.branch_%s", current_branch);
        fs_cp(".bear_git/.prev", current_branch_file); //???
    }
    ...
    if (is_it_a_commit_id(arg))
    {
        ...
        write_string_to_file(".bear_git/current_branch", "");
    ...
    char *branch_file = ".bear_git/.branch_";
    strcat(branch_file, branch_name);

    // Update the branch file if new branch is created (now it can't go wrong anymore)
    if (new_branch)
    {
        FILE *fbranches = fopen(".bear_git/.branches", "a");
        fprintf(fbranches, "%s\n", branch_name);
        fclose(fbranches);
        fs_cp(".bear_git/.prev", branch_file);
    }

    write_string_to_file(".bear_git/.current_branch", branch_name);
