...
int bear_git_commit(char *msg)
{
    ...
    // Generate the next ID (newid) in such a way that: All characters
    // of the id are either 6, 1 or c;  Generating 100 IDs in a row
    // will generate 100 IDs that are all unique(Hint: you can do this
    // in such a way that you go through all possible IDs before you4
    // repeat yourself.Some of the ideas from the number representation class may help you !) char commit_id[COMMIT_ID_SIZE];
