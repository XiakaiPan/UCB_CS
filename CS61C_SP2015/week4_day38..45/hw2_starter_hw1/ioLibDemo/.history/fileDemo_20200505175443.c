...
int main()
{
	...
	// Can' pass?!!
	char c1, c2, c3;
	fscanf(fptr, "%c%c%c", c1, c2, c3);
	printf("the one to three char of fileDemo.txt is:\n: %c\n%c\n%c\n", c1, c2, c3);

	/**
	 * 10. fprintf():
	 * return the bytes which has been written.
	 */
	FILE *fp4 = fopen("fprintf.txt", "w");
	printf("the number of writing is: %d", fprintf(fp4, "First string: %s;\nSencod string: %s;\n", "Hello, printf1", "Hello, printf2");
	fclose(fp4);
