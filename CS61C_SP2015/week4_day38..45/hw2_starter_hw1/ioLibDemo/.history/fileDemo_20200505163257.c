...
int main()
{
	...
	printf("the count is %d", fwrite(fptr, sizeof(char), 3, fp1));
	 ...
	 */
	char c = fgetc(fptr);
	printf("the first char of fileDemo.txt is: %c", c);

	/**
	 * 6.
	 */
	FILE *fp2 = fopen("fputc.txt", "r");
	printf("The ascii code of char which is written into %s: %c", "fputc.txt", fputc(c, fp2));
