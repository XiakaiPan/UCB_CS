...
def make_repeater(h, n):
    ...
    if n == 0:
        return h
    return 
    #this answer can be achieved by using compose1
    '''
    def f(x):
        return repeat(x)
    def repeat(x):
        ...
        return x
    return f
    #my answer is redundant, just repeat() is enough.
    #answer
    '''
    g = identity
    while n > 0:
    ...
    if n == 0:
        return lambda x: x
    return compose1(h, make_repeater(f, n-1))
    or return accumulate(compose1, lambda x: x, n, lambda k: h)
    '''
