...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    city = make_city('nw_city', lat, lon)
...
def sprout_leaves(t, values):
    ...
    if is_tree(t):
        print(t[0])
        sprout_leaves(t[1:], values)
        if len(t) > 2:
            sprout_leaves(t[2], values)
    else:
        print("  ", values[0])
        print("  ", values[1])
    '''
...
def partition_options(total, biggest):
    ...
    if total == 0:
        ...
        return [biggest] + partition_options(total-1, biggest)
    else:
        with_biggest = [[biggest] + partition_options(total-biggest, biggest-1)]
        without_biggest = [partition_options(total, biggest-1)]
        without_biggest = [i for i in without_biggest]
