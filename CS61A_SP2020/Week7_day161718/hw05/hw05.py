...
class VendingMachine:
    ...
    def vend(self):
        if self.num <= 0:
            ...
            self.money = 0
            self.num -= 1
            return 'Here is your {0} and ${1} change.'.format(self.product, current_money-self.price)
...
def preorder(t):
    return reduce(add, [preorder1(child) for child in t.branches], [t.label])
'''
...
def is_bst(t):
    ...
    "*** YOUR CODE HERE ***"
    def bst_min(t):
        '''Return the min of t, if it has the structure of a valid BST.'''
        if t.is_leaf():
            return t.label
